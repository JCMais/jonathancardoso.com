...
- Android Studio
- Code Editor
- Ruby basics would be great but not required
...
This variable is going to be used later when we call the plugin method to increment the build number, it's needed because we want to pass an extra param to this method depending if we called our lane with a certain option or not:
...
`Actions.lane_context` is a special hash used by Fatlane that contains values shared from actions that were run previously, in our case we are retrieving the value for the key `Actions::SharedValues::ANDROID_VERSION_CODE`.
...
This lane follows the same idea than the previous one. But it accepts a few more options. The most important ones being `increment_type` and `version`.

We can pass either of them. `version` allow us to specify the new version number directly, while `increment_type` give us the option to just say the increment type, which can be one of: `patch`, `minor`, and `major`.
...
If we don't pass anything, the default will be an `patch` increment of the current version.
...
![](./images/image-21-fastlane-ivn.png)
...
And then passes those to the [`gradle` action](https://docs.fastlane.tools/actions/gradle/).
...
We then call the [`supply` action](https://docs.fastlane.tools/actions/supply/) passing two option:
...
```bash
git add --all && git commit -m "add fastlane and lanes to publish app to play store"
```
...
_[DSL]: Domain-Specific Language
_[CLI]: Command Line Interface
_[AAB]: Android App Bundle
