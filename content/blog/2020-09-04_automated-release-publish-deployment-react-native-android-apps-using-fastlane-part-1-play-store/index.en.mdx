...
description: Everyone that has been releasing apps manually knows how tiresome and time consuming the process is, the main idea behind this...
...
externalLinks:
  devto: https://dev.to/jonathancardoso/automated-deployment-of-react-native-apps-using-fastlane-part-1-play-store-3n0g
---
...
While this guide is focused on sending React Native apps to the Google Play Store, the same steps can be taken to add fastlane to any android App, be it Flutter, NativeScript or Kotlin/Java.

And the same can be said about submitting to AppCenter instead of the Google Play Store. It can easily be done with just some small changes to the code, using the actions provided by the [fastlane plugin appcenter](https://github.com/microsoft/fastlane-plugin-appcenter) instead of the ones we will be using here.
...
> You may have noted that the environment variables have a common prefix, `MYRNAPP_RELEASE_`, you can change this to anything that would make more sense for your project.  
> The idea of having a prefix is to avoid collisions when you are setting these environment variables for multiple apps on the settings of your CI/CD provider.
